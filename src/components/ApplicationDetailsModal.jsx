...
export default function ApplicationDetailsModal({ app, onClose, onUpdate }) {
  ...
  return (
    <div className="fixed inset-0 bg-columbia/20 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-fade-in transition-all duration-500">
      <div className="bg-white p-10 rounded-[2.5rem] shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col border border-white/50">
        
        <div className="flex justify-between items-start mb-8 shrink-0 border-b border-columbia/10 pb-6">
          <div>
            <h2 className="text-4xl font-black text-cherry tracking-tighter">{app.companyName}</h2>
            <p className="text-xl text-gray-400 font-bold tracking-tight">{app.position}</p>
          </div>
          <button onClick={onClose} className="w-12 h-12 flex items-center justify-center bg-alabaster rounded-full text-gray-300 hover:text-cherry transition-all text-2xl font-bold">×</button>
        </div>
              ...
              {notesList.length === 0 ? (
                ...
                notesList.map((note, idx) => (
                  <div key={idx} className="relative pl-8 border-l-2 border-alabaster group pb-2">
                    <div className="absolute -left-[9px] top-1.5 w-4 h-4 bg-alabaster rounded-full group-hover:bg-cherry transition-all duration-300 shadow-sm" />
                    <span className="text-[9px] font-black text-gray-300 uppercase tracking-widest">{note.date}</span>
                    <p className="text-sm text-gray-600 mt-2 leading-relaxed font-medium">{note.text}</p>
                  </div>
                ))
              )}
            </div>
